...
icmpRdir=$(grep "net\.ipv4\.conf\.all\.accept_redirects" /etc/sysctl.conf /etc/sysctl.d/*)
check "ICMP redirects policy" "net.ipv4.conf.all.accept_redirects= 0" "${icmpRdir}"
icmpRdir=$(grep "net\.ipv4\.conf\.default\.accept_redirects" /etc/sysctl.conf /etc/sysctl.d/*)
check "ICMP redirects policy" "net.ipv4.conf.default.accept_redirects= 0" "${icmpRdir}"

# Check crontab permissions
cronTab=$(stat /etc/crontab)
